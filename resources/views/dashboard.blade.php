...
(function(){
	...
	// inject current user from blade
	const currentUser = {
	...
	accountForm.addEventListener('submit', function(e){
		...
		.then(async res => {
			...
			document.querySelectorAll('.sidebar-profile').forEach(sp => {
				// update name / email inside the sidebar-profile block
				sp.querySelector('div > .fw-bold').textContent = currentUser.name;
				const em = sp.querySelector('div > div.small, div > div:nth-child(2)');
